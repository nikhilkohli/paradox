                        ...
                        <div class="card-body">
                          <!--this form is created for vendor to add the product detail in database-->
                            <form action="admin_insert.php" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label for="">Your dicount Scheme</label>
...
if(isset($_POST['send'])){    /*when admin will press submit button by filling the form records then the all data  will be stored in DB */
    ...
    $query = mysql_query("INSERT INTO product (p_name,p_brand,p_price,p_discount,each_qty,p_mfg,p_img,cat_title,p_desc,Qty,v_email) value ('$p_name','$p_brand','$p_price','$p_discount','$each_qty','$p_mfg','$p_img','$p_cat_title','".mysql_real_escape_string($p_desc)."','$qty','$log')");
    $p_code_update=mysql_query("UPDATE product SET p_code = concat( pstr, p_id )");
